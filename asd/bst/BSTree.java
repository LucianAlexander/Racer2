...
public interface BSTree<E> {
    ...
    /**
     * indica se il nodo � una foglia dell'albero
	 *
	 * @param v
     * @return
     */
...
public class BSTree<V> {
	...
	*/
	...
	/*
		Restituisce la chiave del nodo floor(k)
		(o null se il BST non contiene nessun nodo con chiave <= k).
	*/
	public Integer floor(int k) {
		Node<V> x = floor(root, k);
		if(x == null) return null;
		return x.key;
	}
	...
	/*
		Metodo ausiliario ricorsivo di floor.
		Restituisce il nodo floor(k) calcolato nel sottoalbero la cui radice è x
		(o null se tale sottoalbero non contiene nessun nodo con chiave <= k).
	*/
	private Node<V> floor(Node<V> x, int k) {
		if (x == null) return null;
		if (k == x.key) return x;
		if (k < x.key) return floor(x.left, k);
		Node<V> t = floor(x.right, k);
		if(t != null) return t;
		return x;
	}
